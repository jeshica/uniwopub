...
class DB: 
    ...
    def _connect_with_pooling(self, keywords):
        def get_pooled_db():
            from DBUtils import PooledDB  # @UnresolvedImport
...
class PostgresDB(DB): 
    ...
    def __init__(self, **keywords):
        ...
        if db_module.__name__ == "psycopg2":
            import psycopg2.extensions  # @UnresolvedImport
...
class FirebirdDB(DB):
    ...
    def __init__(self, **keywords):
        try:
            import kinterbasdb as db  # @UnresolvedImport
        except Exception:
            db = None
            pass
        if 'pw' in keywords:
...
class MSSQLDB(DB):
    def __init__(self, **keywords):
        import pymssql as db  # @UnresolvedImport
        if 'pw' in keywords:
            keywords['password'] = keywords.pop('pw')
        keywords['database'] = keywords.pop('db')
        self.dbname = "mssql"
...
class OracleDB(DB): 
    def __init__(self, **keywords): 
        import cx_Oracle as db  # @UnresolvedImport
        if 'pw' in keywords: 
            keywords['password'] = keywords.pop('pw') 
